...
class BookManager:
    ...
    def search_book_by_name(self, book_name,user_id):
        sql = """
        SELECT s.book_id, s.book_name, s.author, s.sort, s.state
        FROM book s
        WHERE s.book_name like %s
        """
    ...
    def search_book_by_sort(self, sort,user_id):
        sql = """
        SELECT s.book_id, s.book_name, s.author, s.sort, s.state
        FROM book s
        WHERE s.sort = %s
        """
        self.cursor.execute(sql, sort)
        res = self.cursor.fetchall()
        if res:
            for i in res:
                print("\n========== 图书信息详情 ==========")
    ...
    def search_book_by_author(self, author,user_id):
        sql = """
        SELECT s.book_id, s.book_name, s.author, s.sort, s.state
        FROM book s
        WHERE s.author = %s
    ...
    def create_user(self,user_name,passwords):
        ...
        res = self.cursor.fetchall()
        for i in res:
            if user_name==i['admin_id']:
                print('用户已存在')
                return
        try:
            sql = "INSERT INTO admin(admin_id,passwords) VALUES(%s,%s)"
            self.cursor.execute(sql, (user_name,passwords))
            self.conn.commit()
            print("✅ 用户注册成功")
        except Exception as e:
            self.conn.rollback()
            print(f"❌ 注册失败！用户id重复或数据格式错误")
    ...
    def is_admin(self,user_id):
        sql="SELECT is_admin FROM admin where admin_id=%s"
...
def main():
    sm = BookManager()
    while True:
        action=input("退出输入0|注册用户输入1|登入系统输入2：")
        if action=='0':
            break
        elif action=='1':
            print("======= 用户注册 =======")
            user_id=input("输入用户名：")
            passwords=input("输入注册密码：")
            sm.create_user(user_id,passwords)
            continue
        elif action=='2':
            print("======= 用户登入 =======")
            user_id=input('输入用户名：')
            if sm.is_users_id(user_id)==():
                ...
                if not sm.is_passwords(user_id,passwords):
                    ...
                    is_admin_answer=sm.is_admin(user_id)
                    print("✅ 登入成功")
                    while True:
                        print("\n======= 图书信息管理系统 =======")
                        print("1. 添加图书信息（仅管理员）")
                        ...
                        if choice == "1":
                            ...
                            sm.search_book_by_name(book_name,user_id)

                        elif choice == "5":
                            sort = input("请输入查询图书分类：")
                            sm.search_book_by_sort(sort,user_id)

                        elif choice == "6":
                            author = input("请输入查询作者：")
                            sm.search_book_by_author(author,user_id)
